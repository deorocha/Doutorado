...
st.set_page_config(
    page_title="Gerador de Texto com Modelo Leve",
    page_icon="🚀",
    layout="wide",
    ...
    try:
        with st.spinner("🔍 Carregando modelo..."):
            ...
            if success:
                ...
                if st.session_state.using_light_model:
                    st.success("✅ Modelo leve carregado com sucesso!")
                else:
...
# Cabeçalho
st.markdown('<h1 class="main-header">🚀 Gerador de Texto com Modelo Leve</h1>', 
           unsafe_allow_html=True)
st.markdown("Gere textos usando modelos de linguagem leves em português")
...
with st.sidebar:
    ...
    if st.session_state.model_loaded:
        if st.session_state.using_light_model:
            st.success("🌐 Modelo Leve Online")
            st.info("Usando modelo pré-treinado leve da internet")
        else:
            st.success("📁 Modelo Local")
            st.info("Usando modelo carregado do diretório local")
    else:
        st.warning("⚠️ Modelo não carregado")
